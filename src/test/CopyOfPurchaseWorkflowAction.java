...
public class CopyOfPurchaseWorkflowAction implements Action {
	...
	public String execute(RequestInfo info) {
		...
		if(!"".equals(tableName)) {
			...
			while(rs.next()){
				...
				String sql_2 = "select id from uf_Purchase where formmodeid=94 and Month = '"
						+Month+"' and Year = '"+Year+"' and type=0";	
				rs1.execute(sql_2);
				if (rs1.next()) {
				...
				if ("".equals(id_1)) {
					...
					String sql_3 = "insert into uf_Purchase(formmodeid,Year,Month,status,sysNo,type) values(94,'" 
							+ Year +"','" + Month + "',0,'"+sysNo+"',0)";
					rs1.execute(sql_3);
					
					// 拿取权限表IDs  formmodeid=94控制只有这表单建模的值，其他引用这个表单的表单建模的值过滤掉了
					String sql_Datashare = "select * from uf_Purchase where formmodeid=94 and Year = '"
							+Year+"' and Month = '"+Month+"' and type=0";
