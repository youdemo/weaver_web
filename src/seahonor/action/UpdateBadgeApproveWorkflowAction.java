...
//海之信申请修改证章审批流程
public class UpdateBadgeApproveWorkflowAction implements Action {

	BaseBean log = new BaseBean();// 定义写入日志的对象

	public String execute(RequestInfo info) {
		log.writeLog("海之信申请修改证章审批流程UpdateBadgeApproveWorkflowAction——————");
		...
		if (!"".equals(tableName)) {
			...
			while (rs.next()) {	
				...
				rs.execute(sql_up);
				log.writeLog("UpdateBadgeApproveWorkflowActionsql_up=" + sql_up);
		} else {
			return "-1";
		}
		return SUCCESS;
