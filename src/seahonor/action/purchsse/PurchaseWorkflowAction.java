	...
	public String execute(RequestInfo info) {
		...
		if(!"".equals(tableName)) {
			...
			while(rs.next()){
				...
				String sql_2 = "select id from uf_Purchase where formmodeid=94 and status = 0 and type=0";	
				rs1.execute(sql_2);
				if (rs1.next()) {
				...
				if ("".equals(id_1)) {
					...
					String sql_3 = "insert into uf_Purchase(formmodeid,status,sysNo,type) values(94,0,'"+sysNo+"',0)";
					rs1.execute(sql_3);
					
					// 拿取权限表IDs  formmodeid=94控制只有这表单建模的值，其他引用这个表单的表单建模的值过滤掉了
					String sql_Datashare = "select * from uf_Purchase where formmodeid=94 and status = 0 and type=0";
				...
				while(rs1.next()) {
					...
					String QueryPrice = "<a href=\''/seahonor/purchase/getSupplierPriceUrl.jsp?deRemark="+tmp_id+"\''>点击进入询价</a>";
