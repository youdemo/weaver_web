...
public class EditContact {
	...
	public String getID(String xgid,String xgseq){
		...
		String billid="";
		String sql="select id from uf_edit_contact where xgid='"+xgid+"' and xgseq='"+xgseq+"'";
		rs.executeSql(sql);
		if(rs.next()){
			billid = rs.getString("id");
		}else{
			insertHistory("-63","uf_Contacts","id", xgid,"uf_edit_contact",xgseq);
			sql="select id from uf_edit_contact where xgid='"+xgid+"' and xgseq='"+xgseq+"'";
			rs.executeSql(sql);
			if(rs.next()){
				billid = rs.getString("id");
				ModeRightInfo ModeRightInfo = new ModeRightInfo();
				ModeRightInfo.editModeDataShare(1, 191,
						Integer.valueOf(billid));//测试173 正式191
			}
		}
		return billid;
 	...
 	//	log.writeLog("insertNow(1) = " + sql);
		...
		//	log.writeLog("insertNow(2) = " + sql);
			...
			if(mapStr.size() > 0){
				...
				mapStr.put("formmodeid", "191");//测试173 正式191
