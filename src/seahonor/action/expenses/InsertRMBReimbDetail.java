...
public class InsertRMBReimbDetail implements Action{
	...
	public String execute(RequestInfo info) {
		...
		mapStr.put("jmid", billId);// 
		mapStr.put("mainid", rqMainid);
		iu.insert(mapStr, flowTableName);
		sql="select * from "+flowTableName+" where mainid="+rqMainid+" and jmid="+billId;
		rs.executeSql(sql);
		if(rs.next()){
			lcmxid = Util.null2String(rs.getString("id"));
		}
		sql="update "+tableName+" set lcmxid='"+lcmxid+"' where id="+billId;
		rs.executeSql(sql);
		return SUCCESS;
	}
