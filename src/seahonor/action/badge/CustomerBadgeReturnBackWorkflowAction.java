...
//海之信申请客户证章退还流程中退回第一节点表单建模状态改变
public class CustomerBadgeReturnBackWorkflowAction implements Action {

	BaseBean log = new BaseBean();// 定义写入日志的对象

	public String execute(RequestInfo info) {
		log.writeLog("进入客户证章退还流程中退回第一节点CustomerBadgeReturnBackWorkflowAction——————");
		...
		/*	while (rs.next()) {	
				...
				log.writeLog("海之信申请退还印章流程CustomerBadgeRet1=" + CustomerBadgeRet1);
				for (int i = 0; i < CustomerBadgeRet1.length; i++) {
					//截取其中的数据，是从0开始算到最后。如32_13
					CustomerBadgeRet = CustomerBadgeRet1[i].substring(3);
					log.writeLog("海之信申请退还印章流程中退回第一节点LendSeal=" + CustomerBadgeRet);
					String sql_up = "update uf_badges set State = '1' where id in (" + CustomerBadgeRet + ")";
					rs.execute(sql_up);
			...
			while (rs.next()) {		
				CustomerBadgeRet = Util.null2String(rs.getString("CustomerBadgeReturn1"));//借出印章
			    String sql_up = "update uf_badges set State = '1' where id in (" + CustomerBadgeRet + ")";
				rs1.execute(sql_up);
				log.writeLog("客户印章流程状态为已退还中更新sql_up=" + sql_up);
			}	
			
				/*//4是代表退还状态
				 //1、正常；2、失效；3、已借出；4、已退还；5、借出中；6、退还中。
