...
public class CopyOfGetWCCImplAction {
	...
	private String[] getDocIds(WCCContactObject wco) {
		...
		while(rsd.next()){
			...
			String rawdatas="";
			sql_dt="select  sys.fn_varbintohexstr(CONVERT (varbinary(max), RawData)) as RawData from WCCContactImageRawData where ContactImageGuid='"+ContactImageGuid+"' order by SubType asc";
			rsd_dt.execute(sql_dt);
			while(rsd_dt.next()){
				rawdatas=rawdatas+Util.null2String(rsd_dt.getString("RawData"));
			}
			rawdatas=rawdatas.replaceAll("0x", "");
			String docname=wco.getName()+sns.getNum("ZP", "WCC_ZP", 5)+".jpg";
			String docid="";
			log.writeLog("图片name:"+docname+" value:"+rawdatas);
			try {
				docid = getDocId( docname,rawdatas,"1");
			} catch (Exception e) {
